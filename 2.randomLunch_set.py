# 두 번째. students를 set에다 넣고 pop을 이용함 -> 해쉬맵을 이용하니까 더 빠른 연산이 가능하지 않을까 싶다
...
# 1. set 클래스로 만들기
# 오히려 set클래스는 random도 필요없고 연산도 간단하지 않나 싶습니다
students = set(students)  # 나중에 삭제를 간편히 하기 위해 set으로 만들어버림.

# 2. 못오신 분들 제외하기
atHome = input('못오신 분들 이름 적기(스페이스로 구분, 다 오셨으면 그냥 엔터) : ').split()

# 2-1. 이름을 적다가 오타가 난 경우를 대비하자.
while True:
    try:
        for home in atHome:
        ...
        print()
        print(f'{err}에서 오타가 났습니다\n')
        atHome = input('오타난 분 이름부터 다시 입력(스페이스로 구분) : ').split()
    else:
        break


# 3. 남은 인원을 3명,4명으로 나누면 총 몇 조가 나오는지 구하기 -> Greedy 사용
partition_dict = {}
...
# 3-1. 현재 인원 수로 4명/3명인 조를 만들 수 없다면?
if studentsLength < 0:
...
# 4. 위의 결과를 이용해 사람 나누기
# 인원별 조의 수를 활용해 사람을 뽑는다(set클래스의 pop메서드 이용) ex) 위의 딕셔너리 결과가 {'3':2, '4':3} -> 3명인 조 2개, 4명인 조 3개
